...
def main(args):
    ...
    args.dataset = "data_cache/Beauty"
    ...
    valid_data_set = SeqDataset_Validation(
        user_train, user_valid, itemnum, args.maxlen,
        validation_candidate_shuffle, validation_user_shuffle)
    test_data_set = SeqDataset_Test(
        user_train, user_valid, user_test, itemnum, args.maxlen,
        test_candidate_shuffle, test_user_shuffle)
    ...
    for epoch in range(epoch_start_idx, args.num_epochs + 1):
        ...
        for step, data in enumerate(train_iterator):
            ...
            # HSTU uses sampled softmax loss (neg is ignored, handled internally)
            loss = model(u, seq, pos_seqs=pos)
        ...
        if epoch % 5 == 0:
            ...
            valid_iterator = tqdm(valid_data_loader, desc="Validation (HR@5=X.X) (NDCG@5=X.X)",
                                  dynamic_ncols=True)
    ...
    critical_indication, simulator_prompt, diagnosis_prompt = get_summarized_suggestion(
        user_feedback_dict, math_diagnosis, math_results, 'deepseek-v4-pro', aggregator_prompt)
