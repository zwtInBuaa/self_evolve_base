...
8. Do NOT expand any single file beyond roughly 1000 lines of code.
"""
...
Current Recommendation Model's Code
```{language_recommendation}
{current_recommendation_program}
...
class CoderAgent_Diagnosis:
    ...
    async def run(
        ...
        current_recommender_code: str = None,
    ) -> str:
        ...
        concatenated_code = current_recommender_code
        cleaned_code = re.sub(r"```[\w]*\n", "", concatenated_code)
        cleaned_code = re.sub(r"```", "", cleaned_code)
        pattern = re.compile(r"# === (.+?) ===\n(.*?)(?=(?:# === .+? ===\n)|\Z)", re.DOTALL)
        matches = pattern.findall(cleaned_code)
        ...
        for filename, code_content in matches:
            if 'models' in filename:
                current_recommender_code  = f"# === {filename} ===\n{code_content}"
        ...
        with trace(f"Self_EvolveRec_{self.problem_name}", trace_id=trace_id, disabled=False):
            ...
            for ref_idx in range(max_reflection_times + 1):
                ...
                code_prompt = DIFF_CODE_TEMPLATE.format(
                    ...
                    language_recommendation=self.language,
                    current_recommendation_program=current_recommender_code,
                    language=self.language,
